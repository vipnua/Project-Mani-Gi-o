...
import { Product } from '../models/products';
 ...
 const Home ={
    async render(){
        ...
        let Cellphone;
        const category: Product[] = data.data.product;
        ...
        if(search){
            ...
            let cellphone = datacellphone.data;
            Cellphone=cellphone;     
            ...
            if(Cellphone.product){
                Cellphone = Cellphone.product
            }
            else{
                Cellphone
            }
        ...
        ${Cellphone.map(cell=> /*html*/`
               <a class="hover:bg-gray-100 rounded hover:drop-shadow-xl" href="/#/product/${cell._id}"data-navigo>
               ...
               <div class="flex justify-center p-2 ">
                 <img class="" src="${cell.images.thumbnail?cell.images.thumbnail:cell.images[0].thumbnail}" width="160px" >  
               </div>
               ...
               <div class="flex"> 
               <div id="rater" data-rate-value=""></div>
               </div>
           </div>
        ...
        <div class="grid grid-cols-10">
            ...
            <div class="bg-[#FFA3A3] rounded-xl m-2">
            <span class="text-white text-sm pl-2 pb-7">${data.data.name}</span>
            <div class="flex justify-center py-2"><img src="${data.data.image}"></div>
        </div>
    ...
    async afterRender(){
        ...
        const searchStates = async (searchtext: string) =>{
                let states = await getAll();
                    let matches = states.data.product.filter((state: { name: string; abbr: string; })=>{
        ...
        const outputHtml = (matches: { lenght: number; map: (arg0: (match: any) => string) => any; }) => {
            if(matches){
                ...
                const html = matches.map(match => `
                          <a href="#/product/${match._id}" data-navigo"> 
        ...
        for (const categories of category) {
            ...
            {e.preventDefault()
                ...
                if(elementcate == 'all'){
                    ...
                    followcate =data.product.filter((item: { category: any } )=> { return item.category === elementcate});
                    localStorage.clear();
                    localStorage.setItem('cellphone',JSON.stringify(followcate))            
                }          
                reRender('#app',Home)
